...
int main(void) {
    ...
    pthread_t system_threads_ID[manager.system_array.size];
    int system_count;

    // Get the count of systems
    system_count = manager.system_array.size;

    // Create manager thread
    pthread_create(&manager_thread_ID, NULL, manager_thread, &manager);
    // Create threads for each system
    for (int i = 0; i < system_count; i++) {
        pthread_create(&system_threads_ID[i], NULL, system_thread, manager.system_array.systems[i]);
    }

    // Join manager thread
    pthread_join(manager_thread_ID, NULL);

    // Join all system threads
    for (int i = 0; i < system_count; i++) {
        pthread_join(system_threads_ID[i], NULL);
    }

    // Cleanup after all threads are joined
