...
void manager_clean(Manager *manager) {
    //Check if the manager pointer is a null pointer
    if (manager == NULL) {
        fprintf(stderr, "Error manager is a null pointer in manager_clean\n");
        return;
    }

    // Clean up the system array
    system_array_clean(&manager->system_array);

    // Clean up the resource array
...
void *manager_thread(void *arg){
    //Makes Manager acccessible to thread function
    Manager *manager = (Manager *)arg;

    // Check if manager is a null pointer
    if (manager == NULL) {
        fprintf(stderr, "Error manager is a null pointer in manager_thread\n");
        return NULL;
    }

    // Run the manager loop until manager simulation_running is 0
